defmodule Aozora.Endpoint.Debit.ClearedTransaction do
  @moduledoc """
  ## 売上確定電文明細照会API
  """
  ...
  def get(account_id, debit_class, from_date, to_date, next_key)
    ...
    url =
      "https://#{Endpoint.api_host() <> Debit.cleared_transaction_path()}?accountId=#{account_id}&requestDebitClass=#{debit_class}&dateFrom=#{from_date}&dateTo=#{to_date}"
  ...
  def get(account_id, debit_class, from_date, to_date, next_key) do
    url =
      "https://#{Endpoint.api_host() <> Debit.cleared_transaction_path()}?accountId=#{account_id}&requestDebitClass=#{debit_class}&dateFrom=#{from_date}&dateTo=#{to_date}&nextItemKey=#{next_key}"
